...
- **导入与导出**：支持社区 PWH 格式和完整保留应用数据的 WWD 格式；PWH 导入前可多选并分配分组，分享入口会按内容自动识别 PWH/WWD。
...
PWH 会一次导出全部转盘，只保留转盘标题以及选项的文字和真实权重；分组、假权重、设置、时间和内部编号均会舍弃。导入确认页支持点选、滑动多选、全选、反选、创建分组和批量分配。Android 分享入口按文件内容识别 PWH 或 WWD，不支持多文件分享。
