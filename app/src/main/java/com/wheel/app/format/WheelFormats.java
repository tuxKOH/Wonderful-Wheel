...
public final class WheelFormats {
    ...
    /** Imports only a window of wheels, allowing callers to keep UI/object allocation bounded. */
    ...
    private static String decodePwh(byte[] raw) throws IOException {
        ...
        try (GZIPInputStream in = new GZIPInputStream(new ByteArrayInputStream(raw, 3, raw.length - 3))) { return new String(readAll(in), StandardCharsets.UTF_8); }
    }
    ...
    private static Map<String, Object> settingsMap(WheelSettings s) {
        ...
        m.put("rotationDurationMs", s.rotationDurationMs); m.put("colorScheme", normalizeColorScheme(s.colorScheme)); m.put("fontSize", s.fontSize);
        ...
        m.put("ttsLanguageTag", s.ttsLanguageTag);
        m.put("textDisplayMode", normalizeTextDisplayMode(s.textDisplayMode));
        m.put("radialTextAutoSize", s.radialTextAutoSize);
        return m;
    }

    private static Map<String, Object> appSettingsMap(AppSettings s) {
        Map<String, Object> m = new LinkedHashMap<>();
        m.put("defaultRotationDurationMs", s.defaultRotationDurationMs); m.put("defaultColorScheme", normalizeColorScheme(s.defaultColorScheme)); m.put("defaultFontSize", s.defaultFontSize);
        m.put("tickSoundEnabled", s.tickSoundEnabled); m.put("selectedSoundEnabled", s.selectedSoundEnabled); m.put("ttsEnabled", s.ttsEnabled);
        m.put("defaultTtsLanguageTag", s.defaultTtsLanguageTag);
        m.put("textDisplayMode", normalizeTextDisplayMode(s.textDisplayMode));
        m.put("radialTextAutoSize", s.radialTextAutoSize);
        m.put("ellipsizeText", s.ellipsizeText);
        return m;
    }
    ...
    private static void readSettings(WheelSettings s, Map<?, ?> m) {
        ...
        s.colorScheme = normalizeColorScheme(str(m.get("colorScheme"), s.colorScheme));
        ...
        s.ttsLanguageTag = str(m.get("ttsLanguageTag"), str(m.get("ttsLocaleTag"), s.ttsLanguageTag));
        s.textDisplayMode = normalizeTextDisplayMode(str(m.get("textDisplayMode"), s.textDisplayMode));
        if (m.get("radialTextAutoSize") instanceof Boolean b) s.radialTextAutoSize = b;
    }

    private static void readAppSettings(AppSettings s, Map<?, ?> m) {
        s.defaultRotationDurationMs = (long) positive(num(m.get("defaultRotationDurationMs"), s.defaultRotationDurationMs), s.defaultRotationDurationMs);
        s.defaultColorScheme = normalizeColorScheme(str(m.get("defaultColorScheme"), s.defaultColorScheme));
        s.defaultFontSize = (int) positive(num(m.get("defaultFontSize"), s.defaultFontSize), s.defaultFontSize);
        if (m.get("tickSoundEnabled") instanceof Boolean b) s.tickSoundEnabled = b;
        if (m.get("selectedSoundEnabled") instanceof Boolean b) s.selectedSoundEnabled = b;
        if (m.get("ttsEnabled") instanceof Boolean b) s.ttsEnabled = b;
        s.defaultTtsLanguageTag = str(m.get("defaultTtsLanguageTag"), str(m.get("ttsLanguageTag"), s.defaultTtsLanguageTag));
        s.textDisplayMode = normalizeTextDisplayMode(str(m.get("textDisplayMode"), s.textDisplayMode));
        if (m.get("radialTextAutoSize") instanceof Boolean b) s.radialTextAutoSize = b;
        if (m.get("ellipsizeText") instanceof Boolean b) s.ellipsizeText = b;
    }

    private static void normalizeSettings(WheelSettings s) {
        s.colorScheme = normalizeColorScheme(s.colorScheme);
        s.textDisplayMode = normalizeTextDisplayMode(s.textDisplayMode);
    }

    public static String normalizeTextDisplayMode(String value) {
        return "radial".equals(value) ? "radial" : "floating";
    }

    public static String normalizeColorScheme(String value) {
        if ("pastel".equals(value) || "vivid".equals(value) || "mono".equals(value)) return value;
        return "classic";
    }
